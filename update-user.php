...
if (sizeof($results) === 0) {
    ...
    if (!empty($errors)) {
        ...
        if ($file['file']['error'] === 4) {
            ...
            $updateQuery = "UPDATE users SET fullname = ?, email = ?, tracking_time_interval = ?, light_version_user = ? WHERE id = ?;";
            mf_do_query($updateQuery, array($fullName, $email, $_POST["tracking_time_interval"], $lighversion, $id), $dbh);
            ...
            if (!in_array($_FILES["file"]["type"], $allowedMimesType) || !in_array($extension, $allowedExts)) {
                ...
                if (!empty($errors)) {
                    ...
                    processUploadedImage($filePath, $minSizeW, $minSizeH, $extension, "U", $fullName, $email, $lighversion, $id);
                }
            }
        }
    }
