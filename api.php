        ...
        switch ($method) {
            ...
            case "doFaceRecognition":
                if (isset($_POST['image_name']) && isset($_POST['email']) &&
                        ...
                        isset($_POST["date"]) && isset($_POST["timezone"]) && 
                        isset($_POST["time"]) && isset($_POST["action"])) {
                    ...
                    if ($row !== false) {
                        ...
                        if (isJson($probability)) {
                            $json_array = json_decode($probability, true);
                            if (array_key_exists("match", $json_array)) {
                                if ($json_array["match"]) {
                                    //Register Check Out.
                                    $insertQuery = "INSERT INTO tracking (email, latitude, longitude, time, date, timestamp, timezone, action) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                                    $dbh = mf_connect_db();
                                    mf_do_query($insertQuery, array($email, $latitude, $longitude, $time, $date, time(), $timezone, $action), $dbh);
                                }
                            }
                        }
                        echo $probability;
